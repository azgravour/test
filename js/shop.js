...
import { state, tg, haptic, open, close, isSoundEnabled, setSoundEnabled, isAutoplayEnabled, setAutoplayEnabled } from './state.js?v20260728s';
import * as Products from './products.js?v20260728s';
import * as Cart from './cart.js?v20260728s';
import * as UI from './ui.js?v20260728s';
import { t, getLang, setLang, applyTranslations } from './i18n.js?v20260728s';
import { registerUser } from './api.js?v20260728s';
import * as Favorites from './favorites.js?v20260728s';
...
function getFilteredProducts() {
  ...
  const minPrice = (p) => (p.variants && p.variants.length) ? Math.min(...p.variants.map(v => v.price)) : Infinity;
  if (state.filters.sort === 'price_asc') list = list.slice().sort((a, b) => minPrice(a) - minPrice(b));
  else if (state.filters.sort === 'price_desc') list = list.slice().sort((a, b) => minPrice(b) - minPrice(a));
...
function quickAddCheapestVariant(productId) {
  ...
  if (inStockVariants.length === 1) {
    // Une seule option possible -> ajout direct, sans étape inutile.
    addToCart(inStockVariants[0].id);
  } else {
    // Plusieurs options -> on laisse le client choisir plutôt que de
    // deviner à sa place laquelle il voulait.
    openProduct(productId);
  }
}

export function refreshGrid() {
  const hasActiveSearch = !!state.filters.query || state.filters.onlyPromo || state.filters.onlyNew || state.filters.onlyFavorites || state.filters.cat !== 'all' || state.filters.sort !== 'default';
...
document.querySelectorAll('[data-sort]').forEach(opt => {
  opt.addEventListener('click', () => {
    document.querySelectorAll('[data-sort]').forEach(o => o.classList.remove('active'));
...
document.getElementById('applyFilter').addEventListener('click', () => {
  ...
  state.filters.sort = document.querySelector('[data-sort].active')?.dataset.sort || 'default';
  document.getElementById('openFilter').classList.toggle('on', state.filters.onlyPromo || state.filters.onlyNew || state.filters.onlyFavorites || state.filters.sort !== 'default');
  refreshGrid();
  close('filter');
  haptic('light');
});
document.getElementById('resetFilter').addEventListener('click', () => {
  ...
  state.filters.onlyPromo = false; state.filters.onlyNew = false; state.filters.onlyFavorites = false; state.filters.sort = 'default';
...
export function refreshCartUI() {
  ...
  document.getElementById('cartBadge').classList.toggle('hidden', qtyTotal === 0);
...
// Trace locale simple de la dernière commande (aucune commande "réelle"
// n'existe côté serveur — la validation se fait manuellement via le lien
// de contact — ceci est juste un rappel pratique pour le client).
const LAST_ORDER_KEY = 'legendlabs_last_order';
function saveLastOrder(lines) {
  const snapshot = {
    date: new Date().toISOString(),
    total: Math.round(lines.reduce((a, l) => a + l.lineTotal, 0) * 100) / 100,
    items: lines.map(l => ({ name: l.product.name, qty: l.qty, unit: l.variant.name || l.variant.unit || '' })),
  };
  try { localStorage.setItem(LAST_ORDER_KEY, JSON.stringify(snapshot)); } catch { /* pas grave */ }
}
...
document.querySelectorAll('.tab').forEach(tab => {
  tab.addEventListener('click', () => {
    ...
    if (key === 'settings') renderProfileView();
  });
});
...
function renderProfileView() {
  ...
  UI.renderLastOrder(getLastOrder());
  const favProducts = state.products.filter(p => Favorites.isFavorite(p.id));
  UI.renderProfileFavorites(favProducts, {
    onOpen: (productId) => {
      document.querySelectorAll('.tab').forEach(t => t.classList.remove('active'));
      document.querySelector('.tab[data-tab="home"]').classList.add('active');
      document.querySelectorAll('.view').forEach(v => v.classList.remove('active'));
    ...
    onRemove: (productId) => { handleFavorite(productId); renderProfileView(); },
    onQuickAdd: (productId) => quickAddCheapestVariant(productId),
  });
}
