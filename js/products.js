...
import { restGet, rpcIncrementLikes } from './api.js?v20260728s';
...
export async function loadProducts() {
  const select = 'products?select=*,media:product_media(*),variants:product_variants(*,lots:product_lots(*)),badges:product_badges(badge:badges(*))&order=sort_order.asc';
  const products = await restGet(select);
  // Aplatit product.badges de [{badge:{...}}] vers [{...}] pour un accès direct.
  products.forEach(p => { p.badges = (p.badges || []).map(pb => pb.badge).filter(Boolean); });
  return products;
}
