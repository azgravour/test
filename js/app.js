...
import { close, haptic, sheets, tg } from './state.js?v20260728s';
import { initShop } from './shop.js?v20260728s';
import { ADMIN_TAP_TRIGGER_COUNT } from './config.js?v20260728s';
import * as UI from './ui.js?v20260728s';
import { submitGlobalPassword } from './access.js?v20260728s';

// ---------- Chargement différé du panneau admin ----------
// admin-panel.js (~48 Ko avec admin.js) n'est téléchargé que si un
// déclenchement à 5 taps est vraiment confirmé — la quasi-totalité des
// visiteurs (clients normaux) ne le charge donc jamais.
let adminModulePromise = null;
function loadAdminModule() {
  if (!adminModulePromise) {
    adminModulePromise = import('./admin-panel.js?v20260728s').then(mod => {
      mod.initAdminPanel();
      return mod;
    });
  }
...
function wireAdminTapTrigger(id) {
  ...
  el.addEventListener('click', () => {
    ...
    tapCount = 0;
    loadAdminModule().then(mod => mod.startAdminFlow());
  });
}
wireAdminTapTrigger('adminTrigger');
wireAdminTapTrigger('maintenanceAdminTrigger');
  ...
  function tryUnlock() {
    ...
    submitGlobalPassword(password)
      .then(() => {
        ...
        haptic('success');
        setTimeout(() => { lockEl.remove(); if (tg) tg.expand(); boot(); }, 400);
      })
      .catch((err) => {
